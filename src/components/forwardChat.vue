 ...
 * @LastEditTime: 2021-06-04 15:09:41
 * @LastEditors: mzr
-->
<template>
    <div class="dialog_forward">
        ...
        <div class="forward_box" @click="openforward">
            <!-- 标题 -->
            <p v-if="dialogList.groupMessageForward">群聊的聊天记录</p>
            <p v-else-if="dialogList.singleForward">{{dialogList.name}}的聊天记录</p>
            <p v-else>{{dialogList.masterName}}和{{dialogList.name}}的聊天记录</p>
            ...
            <div class="box_item" v-for="item in numberList" :key="item">
                ...
                <p class="item_text" v-if="forwardList[item] && forwardList[item].type === 'text'">
                    {{dialogList.name}}:{{JSON.parse(forwardList[item].content).content}}
                </p>
            </div>
        </div>
        ...
        <el-dialog custom-class="forwardDialog" :visible.sync="showDialog" width="520px" append-to-body center>
            <!-- 标题 -->
            <div slot="title" class="forward_title" v-if="dialogList.groupMessageForward">群聊的聊天记录</div>
            <div slot="title" v-else-if="dialogList.singleForward">{{dialogList.name}}的聊天记录</div>
            <div slot="title" v-else>{{dialogList.masterName}}和{{dialogList.name}}的聊天记录</div>
            <!-- 消息列表 -->
